...
function ranking(req, res) {
  ...
  const ganadores = qMeses.all(mes).map(({ mes: m }) => {
    ...
    return { mes: m, nombre: g.nombre, puntaje: g.puntaje };
  });
  responder(res, 200, { mes, top, ganadores }, { "Cache-Control": "no-store" });
}
...
function adminDatos(req, res, url) {
  ...
  responder(res, 200, { mes, top }, { "Cache-Control": "no-store" });
}
